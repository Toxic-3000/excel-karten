...
/* Spieleliste Webansicht – Clean Rebuild – Build V7_1j63u
...
(() => {
  "use strict";
  const BUILD = (document.querySelector('meta[name="app-build"]')?.getAttribute("content") || "V7_1j63u").trim();
  ...
  const srcBadge = badge("source " + classifySource(src), srcLabel);
      ...
      return `
        <article class="card${expandedClass}${detailClass}" data-id="${esc(id || "")}">
          ...
          <div class="chevRow chevRow-detail" aria-hidden="false">
            <button class="chevBtn chevDetail" type="button" aria-label="Detailbereich öffnen/schließen" aria-expanded="${isDetail ? 'true' : 'false'}">${isDetail ? '▴' : '▾'}</button>
          </div>
